...
def prepare_pkg_list():
    global pkglst
    logger.debug('---------------: call prepare_pkg_list')
...
def prepare_apk_list_main():
    """_summary_ Find all files in the apk install directory, store its names in the global apk list var "Lapk" of str
...
def get_radio_modules_qty():
    ...
    if RMq > 0:
        ...
        a = torun + 'shell getprop gsm.sim.state'
        p = (subprocess.run(a, capture_output=True, text=True, shell=True))
        pp = re.findall(',', p.stdout)
        RMq = len(pp)+1
        if RMq < 1:
            logger.error('RadioModules = ' + str(RMq))
            sys.exit(1)
        else: #RMq > 0
            for j in range(RMq):
                devdata.append(
...
def get_NetOps():
    ...
    pp = re.split(',', a)
    if len(a) < 2:
        s = torun + 'shell getprop gsm.sim.operator.alpha'
        p = (subprocess.run(s, capture_output=True, text=True, shell=True))
        a = p.stdout
        a = a.replace('\n', '')  # clean from '\n'
        pp = re.split(',', a)
        ...
        NetOps = pp
    logger.info(f'Connected mobile network: {NetOps}')
    return NetOps
...
def get_iphonesubinfo_main():
    ...
    toshell = torun + ' shell '
    logging.info('call get_iphonesubinfo_main()')
    for i in range(30):
        ...
        n = 0 # no sim loaded
        if len(sim_state) >1:
            if sim_state[0] == 'LOADED' or sim_state[0] == 'READY': # sim #0 loaded
                n = 0
            elif sim_state[1] == 'LOADED' or sim_state[0] == 'READY': # no sim #0, loaded sim #1
                n = 1
...
def get_wifi_data_from_adb_device_main():
    ...
    k = 7
    # [7][0] = WiFi levels
    logger.debug(f'# {k}[0] WiFi levels (shell dumpsys wifi | grep mWifiInfo):')
    # PARAM_1c = 'shell "dumpsys wifi | grep mWifiInfo"'      # for ADB WiFi data inflating to devdata[0]['wRSSI']
    ...
    # [7][1] = WiFi state
    logger.debug(f'# {k}[1] WiFi State:')
    parm = ' shell "dumpsys wifi |grep Wi-Fi"'
    ...
    if p.returncode == 0:
        if p.stdout:
            ...
            logger.debug(f'# ERROR: no data in p.stdout ( = {p.stdout} )')
            procdata[k].append('NONE')  # ----> [procdata[7][1]
    else:
        logger.debug(f'# ERROR: {p.stderr}')
        procdata[k].append('NONE')  # ----> [procdata[7][1]
...
def proceed_adb_device_data():
    ...
    for j in range(RMq):  # for each Radio Module:
        ...
        if get_value_from_string(procdata[0][j], 'mChannelNumber=', ','):
            devdata[j]['mChan'] = get_value_from_string(procdata[0][j], 'mChannelNumber=', ',')
        if get_value_from_string(procdata[0][j], 'mCellBandwidths=[', ','):
        ...
        if get_value_from_string(procdata[0][j], 'RilVoiceRadioTechnology=', ','):
            devdata[j]['mCSRT'] = get_value_from_string(procdata[0][j], 'RilVoiceRadioTechnology=', ',')
        if get_value_from_string(procdata[0][j], 'RilDataRadioTechnology=', ','):
                ...
                if len(sss[j]) > 10:
                    if 'LTE' in Nettype[j]:
                        ...
                        if sss[j][2] != UNKNOWN_VALUE:
                            ...
                            devdata[j]['BER'] = 'N/A'
                # for Androidver <9
                if 4 < intAndroidver < 9: # Android version must be >= 5!
                    logger.debug('Android ver <9 =' + Androidver + 'ServStates processing')
                    if len(ServStates[j]) >12:
...
# class ADB_App_Main. Methods
    ...
    def ADB_device_connect(self):
        """
        Inflate information from connected device
        :return: all returned vars are global
        """
