...
# class ADB_App_Main. Methods
    ...
    def periodic_iperf_client_timeout(self):
        logger.debug('---------------: call periodic_iperf_client_timeout')
        self.periodic_iperf_client_stop()
        self.label_watch.setHidden(True)
        s = self.kill_iperf_client()
        raise subprocess.TimeoutExpired(torun, TIMEOUT_IPERF_CLIENT) # Watchdog2

    def periodic_iperf_client_check_start(self):
        time_w = int(TIMEOUT_IPERF_CLIENT * 1000)
    ...
    def periodic_iperf_client_check_restart(self):
        time_w = int(TIMEOUT_IPERF_CLIENT * 1000)
    ...
    def periodic_iperf_client_stop(self):
        logger.debug('---------------: call periodic_iperf_client_stop')
        self.timer_iperf.stop()
        self.label_watch.setHidden(True)
            ...
            if len(pids)>0:
                for pid in pids:
                    ...
                    if p.returncode > 0:
                        ...
                        logger.debug('---------------: Output: ' + p.stdout)
                        killresult = False
                        logging.debug(f'kill PID:{pid} returncode:{p.returncode}')
                    else:
...
# Try getting Android version (debugging bridge rights checking)
    ...
    ### cmd output
        ...
        watchdog = Watchdog(timeout_parm=TIMEOUT_IPERF_CLIENT+10)
        # self.periodic_iperf_client_check_start()

        logger.info(f'---------------: Watchdog timer started for {TIMEOUT_IPERF_CLIENT} sec.')
        try:
            process = Popen(cmd, stdout=PIPE, stderr=STDOUT, universal_newlines=True,bufsize=1) # , shell=True
            for output in process.stdout:
                ...
                watchdog.reset()
                # self.periodic_iperf_client_check_restart()
            ...
            watchdog.stop()
            self.kill_iperf_client()

            process.kill()  # terminate windows subprocess of adb shell
            self.periodic_iperf_client_stop()

            break_str = '--------------------- [timeout expired] ---------------------'
            logger.info(break_str)
            self.display_iperf_client_output(break_str)
        ...
        if sshclient and sshconn:
            ...
            if (iperf_tcp_time != 'sec' and
                ...
                if self.iperf_version == 3: # iperf3
                    ...
                    # iperf_client_threads = '" | find "SUM' if self.checkBox_sum.checkState() else '' # works better than grep for dynamical output, required shell
                    iperf_client_threads = ' | grep SUM' if self.checkBox_sum.checkState() else '' # huge but work without shell= True
