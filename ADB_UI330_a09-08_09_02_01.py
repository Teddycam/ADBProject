...
# 5000 iperf3.x 1st universal (-s -D) # = IPERF3_PORT
# 5010 iperf2.1.x tcp (-s -D )      # = IPERF3_PORT +10
# 5011 iperf2.1.x udp (-s -D -u)    # = IPERF3_PORT +20
...
TIMEOUT_IPERF_CLIENT = 5 # Default watchdog timeout for iperf client in seconds
...
def adb_data_signal_measurements_reinit_main():
    ...
    global lst_l1, lst_l2, tv1_lst1, tv1_lst2, tv1_lst3, tv2_lst1, tv2_lst2, tv2_lst3,\
        SignalStrength, SS, sss, ip, mip, mmtu, wip, mif, wif, RMq, p6, procdata, devdata
    ...
    # procdata = []  # returned ADB strings, never clear its!
    for i in [0, 1, 3]:
        procdata[i] = []
    # lst_l1 = ['']
    # lst_l2 = ['']
...
def get_fata_from_adb_device_connect_main_01():
    """ 
    ...
    global k, RMq, procdata, devdata, NetPLMNs, NetOps, intAndroidver, Androidver, DevModel, DevMan, DevDev, DevMod,\
        DevName, DevName1, Nettype, datastr, SDK_ver, sim_state, imei, imsi, isdn, iphonesubinfo_registers, abi_list, \
        PKG_DIR, Device_SN
    ...
    #
    a = torun + 'shell "dumpsys telephony.registry |grep ' + '\47' + 'Phone Id=' + '\47\42'
    ...
    for i in range(len(iphonesubinfo_registers)):
        ...
        if res1:
            if i in [1,2]: # can be IMEI
                ...
                if imsi[0]:
                    if imsi[0] != iphonesubinfo_registers[i]:
                        imsi[1] = iphonesubinfo_registers[i]
                else:
                    imsi[0] = iphonesubinfo_registers[i]
            elif i in [15, 16]: # MSISDN
                if sim_state[0] == 'LOADED':
                    n = 0
                else:
                    if sim_state[1] == 'LOADED':
                        n = 1
                ...
                if nbr:
                    if not imsi[0]:
                        ...
                        if (not imsi[1] and nbr not in imsi[0]):
                            imsi[1] = nbr
            logging.debug(f'{i} \t {iphonesubinfo_registers[i]}')
    logging.debug('==================done======================')
...
def get_signal_measurement_data_from_adb():
    ...
    for i in [0, 1, 3]:
        ...
        #   s = torun+'shell "dumpsys telephony.registry |grep '+'\47'+PARAM_2[i]+'\47\42'
        s = torun + PARAM_1a + '\47' + PARAM_2[i] + '\47\42'
        #   p=(subprocess.run(torun+PARAM_1+PARAM_2[i],capture_output=True, text=True, shell = True))
        p = (subprocess.run(s, capture_output=True, text=True, shell=True))
        ...
        if  1 < len(pp[0]) < 5:
            del (pp[0])
        if Measonly: # skip procdata[2],[4]
            procdata[i] = pp
        else:
            procdata.append(pp)
        if i > 0: # procdata[1] do not skipped
            if not Measonly:
                procdata.append('')  # procdata[2]&[4] skipped, not required for meas ( speed!)
        logger.debug(str(i) + ' ' + str(procdata[i]))

# i == 3 there
    if not Measonly: # skip procdata[4]
        procdata.append('')  # 4 skipped, not required for meas ( speed!)
...
def get_fata_from_adb_device_connect_main_02():
    ...
    global k, p6, RMq, procdata, mif, mmtu, mip, wif, wip, datastr, dclnk
    #################
    # 6) interfaces
    #################
    dclnk = []
...
def get_wifi_scanner_data_main():
    ...
    if DevModel:
        ...
        s = torun + PARAM_1b + '\47' + param2 + '\47\42'
        p = (subprocess.run(s, capture_output=True, text=True, shell=True))
...
def get_fata_from_adb_device_connect_main_03():
    """
    This procedure inflating radio modules states and signal level to global vars  from connected DUT via ADB::
    Output (global):
    k - procdata index
    ...
    #################
    k = 7
    procdata.append([])  # create procdata[7]
    ...
    k = 8
    logger.debug('#' + str(k) + ')')
    parm = ' shell df /storage/emulated -H'
    p = (subprocess.run(torun + parm, capture_output=True, text=True, shell=True))
    procdata.append(re.split("\r|\n", p.stdout))
    if len(p.stdout) > 2:
...
#   #################
#   # -) WiFi Scanner data - moved to procedure ADB_device_WiFiScanner_main()
#   #################
...
#    parm = 'shell  "dumpsys meminfo |grep '
#
#    s = torun + parm + '\47' + 'RAM' + '\47\42'
#    p = (subprocess.run(s, capture_output=True, text=True, shell=True))
#    procdata.append(re.split("\r|\n", p.stdout))
#    for i in len(procdata[10]):
...
# devdata fill to lst_l1
    lst_l1[0] = f'Connected: {DevMan} {DevDev} {DevMod} [{DevName} {DevName1}]. Supported ABI: {abi_list}, ' \
                f'\nDevice S/N = {Device_SN}'
    lst_l1.append(f'NR {nr_support}. Android version: {Androidver}, SDK level {SDK_ver}')
    lst_l1.append(f'Radio Module #1: SIM card:{sim_state[0]}  \nIMSI:{imsi[0]}, IMEI:{imei[0]}, MSISDN:{isdn[0]}')
    j = 0  # RadioModule #1
    for b in dict.keys(devdata[j]):
    ...
    if RMq > 1:
        lst_l2[0] = f'Connected: {DevMan} {DevDev} {DevMod} [{DevName} {DevName1}]. Supported ABI: {abi_list}' \
                f'\nDevice S/N = {Device_SN}'
        lst_l2.append(f'NR {nr_support}. Android version: {Androidver}, SDK level {SDK_ver}')
        state = 'NONE' if len(sim_state)<2 else sim_state[1]
        lst_l2.append(f'Radio Module #2: SIM card:{state}, \nIMSI:{imsi[1]}, IMEI:{imei[1]}, MSISDN:{isdn[1]}')
        j = 1  # RadioModule #2
        for b in dict.keys(devdata[j]):
            if str(b) not in LEVELS:
...
class ADB_App_Main(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self, *args, obj=None, **kwargs):
        ...
        self.timer_iperf.timeout.connect(self.timeout_iperf)
...
# class ADB_App_Main. Methods
    ...
    def append_records_lst_l1(self):
        ...
        for row in range(len(lst_l1)):
            self.textEdit_1.append(lst_l1[row])
...
# Lst_l2 append (10)
    def append_records_lst_l2(self):
        ...
        for row in range(len(lst_l2)):
            self.textEdit_2.append(lst_l2[row])
...
# Try getting Android version (debugging bridge rights checking)
    ...
    # Message Box "authorize ADB debugging"
        ...
        get_fata_from_adb_device_connect_main_01()
        ...
        get_fata_from_adb_device_connect_main_02()
        self.progressBar_ADB.setValue(50)  # ====================================================================> 50%
        self.progressBar_ADB.setValue(60)  # ====================================================================> 60%
        get_fata_from_adb_device_connect_main_03()
    ...
    def kill_iperf_client(self):
        ...
        logger.info(f'---------------: called kill_iperf_client iperf version:{version}' )
        iperf_v = iperf2_version if self.iperf_version == 2 else iperf3_version
        s = torun + 'shell pidof '+ iperf2_v
        p = (subprocess.run(s, capture_output=True, text=True, shell=True))
        if p.returncode > 0:
            logger.debug('---------------: Error: ' + p.stderr)
            logger.debug('---------------: Output: ' + p.stdout)
            ...
            pid = p.stdout.replace(' ','').replace('\n','')
            if pid.isdigit():
                print(f'Good PID:{pid}')
            else:
                print(f'Bad PID:{pid}')
        s = torun + 'shell kill '+ pid
        p = (subprocess.run(s, capture_output=True, text=True, shell=True))
        if p.returncode > 0:
            logger.debug('---------------: Error: ' + p.stderr)
            logger.debug('---------------: Output: ' + p.stdout)
    ...
    def run_iperf_clnt_interactive(self, cmd, title = 'undefined test'):
        ...
        title:str title display before client start
            Signals imported from module "signal" that can be used for break:
                ...
                NSIG = 23
                SIGABRT = <signal.SIGABRT: 22>
                SIGBREAK = <signal.SIGBREAK: 21>
                SIGFPE = <signal.SIGFPE: 8>
                SIGILL = <signal.SIGILL: 4>
                SIGINT = <signal.SIGINT: 2>
                SIGSEGV = <signal.SIGSEGV: 11>
    ...
    ### client run
        ...
        with Popen(cmd, stdout=PIPE, stderr=STDOUT, universal_newlines=True) as process:  # text mode


        # with Popen(cmd, stdout=PIPE, stderr=STDOUT, text = True, shell = True) as process:  # text mode

            # kill process in timeout seconds unless the timer is restarted
            watchdog = WatchdogTimer(TIMEOUT_IPERF_CLIENT, callback=process.kill, daemon=True)

            watchdog.start()
            self.label_watch.setHidden(False)
            logger.info(f'---------------: Watchdog timer started for {TIMEOUT_IPERF_CLIENT} sec.')
            ## -------- watchdog started
            ...
            for output in process.stdout:
                ...
                if self.iperf_break:
                    ...
                    self.display_iperf_client_output(res_kill)
                ...
                with watchdog.blocked:
                    self.label_watch.setHidden(True)
                    if True: # not output:
                        # break_str = '--------------------- [Terminated] ---------------------'
                        # logger.info(break_str)
                        # self.display_iperf_client_output(break_str)
                        # process.send_signal(signal.CTRL_C_EVENT)
                        res_kill = kill_iperf_client()  # kill iperf_x process at Android device
                        self.display_iperf_client_output(res_kill)
                        process.terminate() # terminate windows subprocess of adb shell
                        logger.info(f'---------------: PROCESS KILLED BY WATCHDOG')
                        ## ---------------- watchdog restart
                        watchdog.restart()  # restart timer just before reading the next line
                    self.label_watch.setHidden(False)
                    logger.info(f'---------------: Watchdog timer restarted for {TIMEOUT_IPERF_CLIENT} sec.')
            ## -------- watchdog cancelled
            watchdog.cancel()
            logger.info(f'---------------: Watchdog timer cancelled.')
        ...
        if sshclient and sshconn:
            ...
            if (iperf_tcp_time != 'sec' and
                ...
                if self.iperf_version == 3: # iperf3
                    ...
                    iperf_client_threads = '" | find "SUM' if self.checkBox_sum.checkState() else '' # better than grep for dynamical output
                    # iperf_client_threads = ' | grep SUM' if self.checkBox_sum.checkState() else '' # huge
                ...
                iperf_client_cmd_tcp_base = (iperf_client_tcp_cmd0 + ' -t' + iperf_tcp_time
                                                + iperf_tcp1)
...
# c:\adb\adb.exe shell getprop wifi.interface
# wlan0
# c:\adb\adb.exe shell getprop persist.radio.imei
# [persist.radio.imei]: [869773042935512]
# [persist.radio.imei1]: [869773042935512]
# [persist.radio.imei2]: [869773042935520]
# [gsm.sim.state]: [LOADED,ABSENT]
# [gsm.rssi.sim1]: [-79]
# [gsm.sim.operator.numeric]: [25050,]
# [gsm.sim.preiccid_0]: [8971100]
# [gsm.sim.preiccid_1]: []
# [gsm.sim.state]: [LOADED,ABSENT]
# c:\adb\adb.exe get-serialno
# fe848f89

### Xiaomi Mi10:
# >c:\adb\adb.exe shell service call iphonesubinfo 8
# Result: Parcel(
#   0x00000000: 00000000 0000000f 00350032 00350030 '........2.5.0.5.'
#   0x00000010: 00300030 00300031 00380030 00300032 '0.0.1.0.0.8.2.0.'
#   0x00000020: 00390031 00000035                   '1.9.5...        ')
#
### Honor50:
###########################

#  c:\adb\adb.exe  shell   "service call iphonesubinfo 1 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done"
# 861997050130199

#  c:\adb\adb.exe  shell   "service call iphonesubinfo 2 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done"
# 861997050130199

#  c:\adb\adb.exe  shell   "service call iphonesubinfo 4 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done"
# 861997050130199
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 5 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done"
# 861997050130199
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 6 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done"
# 06
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 7 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done"
# 06
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 8 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done"
# 250500100820195
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 9 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done"
# 250500100820195

#  c:\adb\adb.exe  shell   "service call iphonesubinfo 11 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done"
# FFFFFFFFFFFFFFFF
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 12 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done" ====> ???
# 8971100001008201957F
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 13 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done" ====> ???
# 8971100001008201957F
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 14 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done" ====> ???
# 8971100001008201957F

### Xiaomi Mi9 Lite (my):
###########################

#  c:\adb\adb.exe  shell   "service call iphonesubinfo 1 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done" ====> ISDN??
# 869773042935512

#  c:\adb\adb.exe  shell   "service call iphonesubinfo 3 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done" ====> ISDN??
# 869773042935512
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 4 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done" ====> ISDN??
# 869773042935512
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 5 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done"
# 00
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 6 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done"
# 00
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 7 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done" ====> IMSI
# 250021082920533
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 8 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done"
#
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 9 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done"
# FFFFFFFFFFFFFFFF
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 10 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done" ====> ISDN ???
# 897010210829205337FF
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 11 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done" ====> ISDN ????
# 897010210829205337FF

#  c:\adb\adb.exe  shell   "service call iphonesubinfo 27 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done" =======> IMS
# 5 sip:250021082920533@ims.mnc002.mcc250.3gppnetwork.org
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 28 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done"
# 73000000000000000000
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 29 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done"
# (  pcscf.ims.mnc002.mcc250.3gppnetwork.org


### Samsung S20+ (A12)
##############################################################

#  c:\adb\adb.exe  shell   "service call iphonesubinfo 8 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done" ====> secured
# getSubscriberIdForSubscriber: Package null does not belong to 2000
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 9 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done"
# getSubscriberIdForSubscriber: Package null does not belong to 2000
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 10 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done"
# getSubscriberIdForSubscriber: Package null does not belong to 2000

#  c:\adb\adb.exe  shell   "service call iphonesubinfo 12 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done" ====> ISDN
# 8971100001008202682
#  c:\adb\adb.exe  shell   "service call iphonesubinfo 13 | grep -o '[0-9a-f]\{8\} ' | tail -n+3 | while read a; do echo -n \\u${a:4:4}\\u${a:0:4}; done" ====> ISDN
# 8971100001008202682
...
# c:\adb\adb.exe shell getprop ro.serialno
# R58N20X9WHD

# >c:\adb\adb.exe shell df /storage/emulated
#Filesystem     1K-blocks     Used Available Use% Mounted on
# /dev/fuse      113526764 20886140  92509552  19% /storage/emulated